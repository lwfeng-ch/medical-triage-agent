        ...
        if failed_tests > 0:
            ...
            for result in self.results:
                if not result.passed:
                    ...
                    if result.details:
                        report.append(
                            f"    详情: {json.dumps(result.details, ensure_ascii=False, indent=6)}"
                        )
...
class TestRagAgentUnit:
    ...
    def test_filter_messages_normal(self):
        ...
        assert all(
            isinstance(m, (HumanMessage, AIMessage, ToolMessage)) for m in result
    ...
    def test_ragagent_error_hierarchy(self):
        ...
        assert issubclass(
            GraphBuildError, RagAgentError
        ), "GraphBuildError 应继承自 RagAgentError"
        assert issubclass(
            ResponseExtractionError, RagAgentError
        ), "ResponseExtractionError 应继承自 RagAgentError"
        assert issubclass(
            MedicalAnalysisError, RagAgentError
        ), "MedicalAnalysisError 应继承自 RagAgentError"
        assert issubclass(
            ToolExecutionError, RagAgentError
    ...
    def test_ragagent_error_to_dict(self):
        ...
        error = RagAgentError(
            message="测试错误", code="TEST_ERROR", details={"key": "value"}
        )
...
class TestMainUnit:
    ...
    def test_medical_extension_model(self):
        ...
        medical = MedicalExtension(
            risk_level="high", risk_warning="高危警告", disclaimer="免责声明"
        )
    ...
    def test_build_medical_extension_general_route(self):
        ...
        final_payload = {"route": "general", "risk_level": "low"}
...
class TestIntegration:
    ...
    def test_ragagent_to_main_data_flow(self):
        ...
        assert (
            medical_ext.structured_data.triage.urgency_level
            == final_payload["structured_data"]["triage"]["urgency_level"]
        ), "紧急程度应一致"
    ...
    def test_message_filtering_chain(self):
        ...
        messages = [
            ...
            HumanMessage(content="用户问题2"),
            AIMessage(
                content="", tool_calls=[{"id": "call_1", "name": "search", "args": {}}]
            ),
    ...
    def test_error_propagation_chain(self):
        ...
        error = ResponseExtractionError(
            message="响应提取失败", details={"cause": "测试"}
        )
...
if __name__ == "__main__":
    pytest.main([__file__, "-v", "--tb=short", "--durations=20", "-x"])
