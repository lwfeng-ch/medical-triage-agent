...
def run_test():
    ...
    try:
        ...
        mock_docs = [
            Document(
                page_content="苹果公司今天发布了最新的 iPhone 16，具备强大的 AI 功能。"
            ),  # 彻底的干扰项
            Document(
                page_content="量子力学是研究微观粒子运动规律的物理学分支。"
            ),  # 彻底的干扰项
            Document(
                page_content="患者张三，男，45岁，既往有高血压病史5年，目前服用降压药控制良好。"
            ),  # 相关病史
            Document(
                page_content="李四的体检报告显示：空腹血糖偏高，建议低糖饮食，增加运动。"
            ),  # 弱相关
            Document(
                page_content="高血压患者日常饮食应以低盐低脂为主，多吃新鲜蔬菜，避免熬夜。"
            ),  # 强相关目标
            Document(
                page_content="王五，女，体检心电图提示窦性心律不齐，建议心内科随诊。"
            ),  # 其他医疗数据
        ]
        ...
        # 提取两个工具
        retriever_tool = next(
            (t for t in tools if t.name == "health_record_retriever"), None
        )
